...
close F;
my %pass;
open (F,"../01.freeratio/All.freeratio.result.out")||die"$!";
while (<F>) {
    chomp;
    my @a=split(/\s+/,$_);
...
for my $k1 (sort keys %h){
    ...
    for my $k2 (@k2){
        ...
        my $p=`/home/tibet_loach/bio/paml/paml4.8/src/chi2 $deltanp $deltalnl`;
        chomp $p;
