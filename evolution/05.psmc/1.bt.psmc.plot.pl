...
die "$0 u g 1.bt.psmc 2.bt.psmc ... n.bt.psmc\n"unless @psmc;

open(OUT,"> $0.$u.$g.gp");
foreach my $psmc(@psmc){
    my $cmd="perl psmc_plot.g3.pl -R -u $u -g $g $psmc $psmc";
    #`perl psmc_plot.g3.pl -R -u $u -g $g $psmc $psmc`;
    system("$cmd");
    #print "$cmd\n";
}
...
print OUT "
  ...
  set xlab \"Years (g=$g, {/Symbol m}=$u)\" font \"Helvetica,16\";
  ...
  set style line 6 lt 1 lc rgb \"#89C876\" lw 4;
  ...
  set style line 21 lt 1 lc rgb \"#FFABAB\" lw 1;
  set style line 22 lt 1 lc rgb \"#D1F0A6\" lw 1;
